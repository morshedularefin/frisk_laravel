...
<div class="sidebar__widget">
    ...
    <div class="sidebar__post-list">
        @php
        $posts = App\Models\Post::orderBy('id','desc')->get()->take(3);
        @endphp
        ...
        <div class="sidebar__post-item">
            ...
            <div class="sidebar__post-content">
                <h5 class="title"><a href="{{ route('post', $post->slug) }}">{{ $post->title }}</a></h5>
                <span class="date"><i class="flaticon-time"></i>{{ $post->created_at->format('M d, Y') }}</span>
            </div>
        </div>
        @endforeach
    </div>
